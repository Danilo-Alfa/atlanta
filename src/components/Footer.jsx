export default function Footer() {
  return (
    <footer className="footer">
      {"\n \n "}
      <div className="cols">
        {"\n "}
        <div className="container flex justify-between f-wrap">
          {"\n "}
          <div className="box box-logo">
            {"\n "}
            <a className="logo" href="/" title="BateForte Materiais para Construção & Madeireira">
              {"\n "}
              <img src="/assets/img-dc6278851a6d.webp" alt="BateForte Materiais para Construção & Madeireira" width="auto" height="auto" />
              {"\n "}
            </a>
            {" \n "}
            <div className="box-logo-socials">
              {"\n "}
              <a href="https://www.facebook.com/bateforte" title="Facebook" aria-label="Facebook" target="_blank" rel="noopener noreferrer">
                {"\n "}
                <i className="icon icon-facebook v-align-middle" />
                {"\n "}
              </a>
              {"\n "}
              <a href="https://www.instagram.com/bateforte_/" title="Instagram" aria-label="Instagram" target="_blank" rel="noopener noreferrer">
                {"\n "}
                <i className="icon icon-instagram v-align-middle" />
                {"\n "}
              </a>
              {"\n "}
              <a href="https://www.tiktok.com/@bateforte" title="TikTok" aria-label="TikTok" target="_blank" rel="noopener noreferrer">
                {"\n "}
                <i className="icon icon-tiktok v-align-middle" />
                {"\n "}
              </a>
              {"\n "}
            </div>
            {"\n "}
          </div>
          {"\n "}
          <div className="box box-categories">
            {"\n "}
            <div className="title">
              {"Categorias "}
              <i className="icon icon-arrow-down sf-hidden" />
            </div>
            {"\n "}
            <div className="overflow">
              {"\n "}
              <ul className="list">
                <li>
                  <a href="#/categoria/cimento" title="Cimento">
                    {"Cimento"}
                  </a>
                </li>
                <li>
                  <a href="#/categoria/areia" title="Areia">
                    {"Areia"}
                  </a>
                </li>
                <li>
                  <a href="#/categoria/ferro" title="Ferro">
                    {"Ferro"}
                  </a>
                </li>
                <li>
                  <a href="#/categoria/eletrica" title="Elétrica">
                    {"Elétrica"}
                  </a>
                </li>
                <li>
                  <a href="#/categoria/hidraulica" title="Hidráulica">
                    {"Hidráulica"}
                  </a>
                </li>
                <li>
                  <a href="#/categoria/madeiras" title="Madeiras">
                    {"Madeiras"}
                  </a>
                </li>
                <li>
                  <a href="#/categoria/telhas" title="Telhas">
                    {"Telhas"}
                  </a>
                </li>
                <li>
                  <a href="#/categoria/tintas" title="Tintas">
                    {"Tintas"}
                  </a>
                </li>
              </ul>
              {"\n "}
            </div>
            {"\n "}
          </div>
          {"\n "}
          <div className="box box-pages">
            {"\n "}
            <div className="title">
              {"Institucional "}
              <i className="icon icon-arrow-down sf-hidden" />
            </div>
            {"\n "}
            <div className="overflow">
              {"\n "}
              <ul className="list">
                {"\n "}
                <li>
                  {"\n "}
                  <a href="https://www.atlantanet.com.br/empresa" title="Empresa">
                    {"Empresa"}
                  </a>
                  {"\n "}
                </li>
                {"\n "}
                <li>
                  {"\n "}
                  <a href="https://www.atlantanet.com.br/seguranca" title="Seguranca">
                    {"Seguranca"}
                  </a>
                  {"\n "}
                </li>
                {"\n "}
                <li>
                  {"\n "}
                  <a href="https://www.atlantanet.com.br/privacidade" title="Política de Privacidade">
                    {"Política de Privacidade"}
                  </a>
                  {"\n "}
                </li>
                {"\n "}
                <li>
                  {"\n "}
                  <a href="https://www.atlantanet.com.br/relatorio-de-transparencia" title="Relatório de Transparência">
                    {"Relatório de Transparência"}
                  </a>
                  {"\n "}
                </li>
                {"\n "}
                <li>
                  {"\n "}
                  <a href="https://www.atlantanet.com.br/contato" title="Fale Conosco">
                    {"Fale Conosco"}
                  </a>
                  {"\n "}
                </li>
                {"\n "}
              </ul>
              {"\n "}
            </div>
            {"\n "}
          </div>
          {"\n "}
          <div className="box box-infos">
            {"\n "}
            <div className="title">
              {"Atendimento "}
              <i className="icon icon-arrow-down sf-hidden" />
            </div>
            {"\n "}
            <div className="overflow">
              {"\n "}
              <ul className="list">
                <li>
                  {"\n "}
                  <a href="https://api.whatsapp.com/send/?phone=5511943259368" title="WhatsApp" target="_blank" rel="noopener noreferrer">
                    {"\n "}
                    <i className="icon icon-whatsapp v-align-middle" />
                    {"\n "}
                    <span>
                      {"(11) 94325-9368"}
                    </span>
                    {"\n "}
                  </a>
                  {"\n "}
                </li>
                {"\n "}
                <li className="hour">
                  {"\n "}
                  <div className="flex">
                    {"\n "}
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                      <path d="M12 2c5.514 0 10 4.486 10 10s-4.486 10-10 10-10-4.486-10-10 4.486-10 10-10zm0-2c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm5.848 12.459c.202.038.202.333.001.372-1.907.361-6.045 1.111-6.547 1.111-.719 0-1.301-.582-1.301-1.301 0-.512.77-5.447 1.125-7.445.034-.192.312-.181.343.014l.985 6.238 5.394 1.011z" />
                    </svg>
                    {"\n "}
                    <span>
                      {"Horário de Atendimento:"}
                    </span>
                    {"\n "}
                  </div>
                  {"\n "}
                  <div className="text">
                    {"\n "}
                    <span>
                      {"Segunda a Sexta 08:00 as 18:00 e Sábado das 08:00 as 12:00"}
                    </span>
                    {"\n "}
                  </div>
                  {"\n "}
                </li>
              </ul>
              {"\n "}
            </div>
            {"\n "}
          </div>
          {"\n \n "}
          <div className="box payments-seals">
            {"\n "}
            <div className="overflow">
              {" \n "}
              <div className="title">
                {"Formas de pagamento"}
              </div>
              {"\n "}
              <ul className="payment-list flex f-wrap">
                {"\n "}
                <li className="payment-form">
                  {"\n "}
                  <img className="lazyload loaded" alt="Cartão Visa - Vindi" src="/assets/img-4fcfba1d7119.webp" data-ll-status="loaded" />
                  {"\n "}
                </li>
                {"\n "}
                <li className="payment-form">
                  {"\n "}
                  <img className="lazyload loaded" alt="Cartão MasterCard - Vindi" src="/assets/img-78fa4608ee50.webp" data-ll-status="loaded" />
                  {"\n "}
                </li>
                {"\n "}
                <li className="payment-form">
                  {"\n "}
                  <img className="lazyload loaded" alt="Cartão Elo - Vindi" src="/assets/img-b517fb144ccd.webp" data-ll-status="loaded" />
                  {"\n "}
                </li>
                {"\n \n "}
                <li className="payment-form">
                  {"\n "}
                  <img className="lazyload loaded" alt="pix" src="/assets/img-80e133f18f9d.webp" data-ll-status="loaded" />
                  {"\n "}
                </li>
                {"\n \n"}
              </ul>
              {" "}
            </div>
            {"\n \n "}
            <div className="overflow">
              {" \n "}
              <div className="title">
                {"Selos de Segurança"}
              </div>
              {"\n "}
              <ul className="security-seals flex f-wrap">
                {" \n \n "}
                <li className="google-seal">
                  {"\n "}
                  <a href="#" target="_blank" rel="noreferrer noopener" title="Google Safe Browsing">
                    {"\n "}
                    <span className="icon icon-shield vertical-align" />
                    {"\n "}
                    <span className="icon icon-google vertical-align" />
                    {"\n "}
                  </a>
                  {"\n "}
                </li>
                {"\n \n \n "}
                <li className="loja-protegida-seal">
                  {"\n "}
                  <center>
                    {"\n "}
                    <a href="#" target="_blank" rel="noopener">
                      {"\n "}
                      <img src="/assets/img-2077ee59210f.gif" alt="Loja Protegida" title="Loja Protegida" border="0" />
                      {"\n "}
                    </a>
                    {"\n "}
                  </center>
                  {"\n "}
                </li>
                {"\n \n \n \n \n \n \n \n \n \n \n \n "}
              </ul>
              {"\n "}
            </div>
            {"\n "}
          </div>
          {"\n "}
        </div>
        {"\n "}
      </div>
      {"\n \n \n \n "}
      <div className="copy">
        {"\n "}
        <div className="container flex align-center flex-column">
          {"\n \n \n "}
          <div className="text">
            {"BateForte Materiais para Construção & Madeireira - © Todos os direitos reservados.\nCNPJ: 00.000.000/0001-00"}
          </div>
          {"\n \n "}
          <div className="tray">
            {"\n "}
            <div className="credits-partner sama-partner">
              {"\n "}
              <span>
                {"Desenvolvimento \n "}
                <a href="https://trademarketing.digital/" rel="noopener" target="_blank">
                  {"\n "}
                  <img src="/assets/img-f4fb0f1e4f54.webp" width="100%" height="auto" alt="Trade Marketing.digital" />
                  {"\n "}
                </a>
                {"\n "}
              </span>
              {"\n "}
            </div>
            {"\n \n "}
            <div data-tray-tst="logo_tray" id="NavLogoTray">
              {"\n "}
              <span className="logotray-message">
                {"Tecnologia "}
                <a href={"https://www.tray.com.br/?utm_source=rodape&utm_medium=lojas&utm_campaign=1018104&utm_term=Varejo"} target="_blank" rel="noopener nofollow">
                  {"Tray"}
                </a>
              </span>
              {"\n "}
              <br className="sf-hidden" />
              {"\n "}
              <span data-tray-tst="page_load_info" className="timelogotray sf-hidden">
                {"\n Load time: 1.067 secs - 15-07-2026 14:10:53 - node: commerce-web1-generic-tmk - store\n "}
              </span>
              {"\n"}
            </div>
            {"\n "}
          </div>
          {"\n "}
        </div>
        {"\n "}
      </div>
      {"\n"}
    </footer>
  )
}
